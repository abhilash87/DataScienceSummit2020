"""
#Date: 04th July 2020
#Author: Abhilash S. Menon
#Emp ID: 721766
#Problem statement: Generate one right sequence through Genetic Programming
"""

#import requisite packages and functions
import numpy as np
import sys

#Initialiazing key variables
nQueens = 8
STOP_CTR = 28
MUTATE = 0.00001
MUTATE_FLAG = True
...
class BoardPosition:
	...
	def setSurvival(self, val):
		self.survival = val
	def __repr__(self):
		return ('sequence ' + str(self.sequence) + ' fitness ' + str(self.fitness) + ' survival ' + str(self.survival) + '\n')
	
def fitness(chromosome = None):
#Max fitness score in nQueen problem is n*n/2 where n equals total columns. Hence in this case it would be 28
...
def generatePopulation(population_size = 100):
	...
	population = [BoardPosition() for i in list(range(population_size))]
	for i in list(range(population_size)):
